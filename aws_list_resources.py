...
def log_error(msg, region, result_collection):
    result_collection["_metadata"]["errors"][region].append(msg)
    print("Error: {}".format(msg))


def analyze_region(region, args, result_collection):
    """
    ...
    try:
        ...
        log_error(msg, region, result_collection)
        return
...
def lambda_handler(event, context):
    # Parse input from the Lambda event
    regions = event.get("regions", "us-east-1").split(",")
    include_resource_types = event.get("include_resource_types", "*").split(",")
    exclude_resource_types = event.get("exclude_resource_types", "").split(",")
    only_show_counts = event.get("only_show_counts", False)
    ...
    class Args:
        def __init__(self):
            ...
            self.only_show_counts = only_show_counts
    ...
    for region in args.regions:
        result_collection = analyze_region(region, args, result_collection)

    # Write the result to a file (or return it directly)
    result_file = "/tmp/resources.json"
    with open(result_file, "w") as out_file:
        json.dump(result_collection, out_file, indent=2, sort_keys=True)
    ...
    return {
        "statusCode": 200,
        "body": json.dumps("Successfully executed and emailed results."),
    }

def send_email(result_file):
    """
    Sends the result file as an email using Amazon SES.
    """
    ...
    response = ses_client.send_email(
        Source="abeciaj23@gmail.com",  # Replace with your verified SES email
        Destination={
            "ToAddresses": ["abeciaj23@gmail.com"],  # Replace with recipient email
        },
        Message={
            "Subject": {"Data": "AWS Resources Report"},
            "Body": {
                "Text": {"Data": f"Attached is the AWS resources report.\n\n{result_content}"},
            },
        },
    )
    return response